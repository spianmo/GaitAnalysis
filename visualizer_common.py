...
def gif_pose(poseFile, basePath):
    ...
    capId = jsonPose[0]['capId']
    partId = jsonPose[0]['partId']

    filename = basePath + partId + '-' + capId + '-PE.gif'
...
def gif_flexext(poseFile, anglesFile, basePath):
    with open(poseFile, 'r') as f:
        jsonPose = json.load(f)
    ...
    capId = jsonPose[0]['capId']
    partId = jsonPose[0]['partId']
    knee_FlexExt = jsonAngles[0]['knee_FlexExt']
    hip_FlexExt = jsonAngles[0]['hip_FlexExt']

    filename = basePath + partId + '-' + capId + '-FE.gif'
    ims = []  # List of images for gif
...
# Saves gif describing flexion/extension angle extraction from side view
def gif_abdadd(poseFile, anglesFile, basePath):
    with open(poseFile, 'r') as f:
        jsonPose = json.load(f)
    with open(anglesFile, 'r') as f:
        jsonAngles = json.load(f)
    ...
    capId = jsonPose[0]['capId']
    partId = jsonPose[0]['partId']
    knee_AbdAdd = jsonAngles[0]['knee_AbdAdd']
    hip_AbdAdd = jsonAngles[0]['hip_AbdAdd']

    filename = basePath + partId + '-' + capId + '-AA.gif'
    ims = []  # List of images for gif
    gs = gridspec.GridSpec(2, 2)
