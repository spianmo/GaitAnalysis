...
def calc_knee_angle_S(hip, knee, ankle, isRightToLeft):
    ...
    if (isRightToLeft and bc[0] < m_ba[0]):
        angle = - angle  # Check if angle should be negative when walking <---
    elif (not isRightToLeft and bc[0] > m_ba[0]):
        angle = - angle  # Check if angle should be negative when walking --->
    return angle.tolist()
...
def calc_hip_angle_S(hip, knee, isRightToLeft):
    ...
    if (isRightToLeft and ab[0] > m_N[0]):
        angle = - angle
    elif (not isRightToLeft and ab[0] < m_N[0]):
        angle = - angle
    return angle.tolist()
